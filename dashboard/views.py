...
def index(request):
    ...
    #total_female = TitanicPassenger.objects.filter(sex="female").count()
    ...
    # df = df[df["Survived"] == 1]
    # embarked_by_class_df = df.groupby(["Embarked", "Pclass"]).size().unstack(fill_value=0) # Renomeado p/ clareza
    
    # ports = embarked_by_class_df.index.tolist()
    # context["ports"] = ports

    # embarked_by_class = embarked_by_class_df.loc[ports, classes].values.tolist()[::-1]
